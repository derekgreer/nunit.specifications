...
SOLUTION = "src/NUnit.Specifications.sln"
...
task :compile => [:versioning] do
	...
	copyOutputFiles "src/NUnit.Specifications/bin/#{CONFIGURATION}", "*.{dll,pdb}", "#{BUILD_PATH}"
end

task :specs do
	mkdir_p "#{REPORTS_PATH}"
	specs = FileList.new("src/NUnit.Specifications.Specs/bin/#{CONFIGURATION}/*.Specs.dll")
	sh "src/packages/NUnit.Runners.2.6.4/tools/nunit-console.exe #{specs}"
end
